...
export class BasketDesiredDeliveryDateComponent implements OnInit, OnChanges {
  ...
  submitForm() {
    ...
    const date: Date = this.form.get('desiredDeliveryDate').value;
    if (date) {
      this.checkoutFacade.setDesiredDeliveryDate(formatISO(date, { representation: 'date' }));
    } else {
      this.checkoutFacade.setDesiredDeliveryDate('');
    }
  }
}
